...
export default function MasterDashboard() {
  ...
  const fetchMetricsFallback = async () => {
    try {
      ...
      const activeJobs = jobs.filter(job => job.status?.trim() === 'Posted').length;
      const completedJobs = jobs.filter(job => job.status?.trim() === 'Completed').length;
      
      const appliedCandidates = candidates.length;
      const selectedCandidates = candidates.filter(c => c.status === 'Selected' || c.status === 'Shortlisted').length;
      ...
      setMetrics({
        ...
        activeJobs,
        completedJobs,
        pendingInterviews: Math.max(0, selectedCandidates - onboardedCandidates)
      });
