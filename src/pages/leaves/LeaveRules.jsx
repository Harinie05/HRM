    ...
    <div className="rounded-lg shadow-sm" style={{ backgroundColor: 'var(--card-bg, #ffffff)' }}>
      ...
      <div className="p-6 border-b ">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-xl font-semibold text-primary">Leave Rules</h2>
            <p className=" mt-1" style={{color: 'var(--text-secondary, #374151)'}}>Configure accrual, carry forward and encashment policies</p>
          </div>
          <button 
      ...
      <div className="overflow-x-auto">
        <table style={{borderColor: 'var(--border-color, #e2e8f0)'}} className="w-full">
          <thead style={{borderColor: 'var(--border-color, #e2e8f0)'}} className="bg-content">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Accrual Settings</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Carry Forward</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Encashment</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Auto Deduction</th>
              ...
              filteredRules.map((rule) => (
                <tr key={rule.id} className="hover:bg-content">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-primary">{rule.accrual_frequency}</div>
                      <div className="text-sm text-muted">{rule.accrual_method}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {rule.encashment_allowed && rule.encashment_rate && (
                      <div className="text-xs text-muted mt-1">Rate: {rule.encashment_rate}%</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      rule.auto_deduct_lop ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-primary"
                    }`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      rule.status === "Active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-primary"
                    }`}>
                      {rule.status || "Active"}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-secondary mb-1">Accrual Frequency</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-secondary mb-1">Accrual Method</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-secondary mb-1">Carry Forward Limit (days)</label>
                <input
                  type="number"
              ...
              {formData.encashment_allowed && (
                <div>
                  <label className="block text-sm font-medium text-secondary mb-1">Encashment Rate (%)</label>
                  <input
                    type="number"
